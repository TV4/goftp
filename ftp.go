...
func (ftp *FTP) List(path string) (files []string, err error) {
	...
	if !strings.HasPrefix(line, StatusFileOK) && !strings.HasPrefix(line, StatusListStartedOK) {
	...
	if !strings.HasPrefix(line, StatusClosingDataConnection) && !strings.HasPrefix(line, StatusActionOK) {
		err = errors.New(line)
		return
	}
